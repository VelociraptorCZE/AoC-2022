const rucksackCompartments = `WjmsdnddnmQPZPPJPL
...
const sumOfPriorities = rucksackCompartments.map(rucksackContent => [rucksackContent.slice(0, rucksackContent.length / 2), rucksackContent.slice(rucksackContent.length / 2)]).map(([firstCompartment, secondCompartment]) => secondCompartment.match(new RegExp(firstCompartment.split("").join("|")))[0]).reduce((carry, commonType) => carry + (commonType.charCodeAt(0) - (commonType === commonType.toLowerCase() ? 96 : 38)), 0);

const sumOfCommonPriorities = rucksackCompartments.reduce((groups, compartment, i) => { groups[parseInt(i / 3)] = [...groups[parseInt(i / 3)] ?? [], compartment]; return groups; }, []).map(([rucksack1, rucksack2, rucksack3]) => rucksack3.match(new RegExp(rucksack1.match(new RegExp(rucksack2.split("").join("|"), "g")).join("|")))[0]).reduce((carry, commonType) => carry + (commonType.charCodeAt(0) - (commonType === commonType.toLowerCase() ? 96 : 38)), 0);

console.log(sumOfPriorities, sumOfCommonPriorities);
